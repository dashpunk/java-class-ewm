...
public class MsPregSaa extends AppBean {
	...
	public void save() throws MXException {
		try {
			if(getMbo().getMboSet("MSTBITENSPREGAO").isEmpty()){
				...
				if (getMbo().getMboSet("MSTBITENSPREGAO").isEmpty()){
					for (int i = 0; ((mbo= getMbo().getMboSet("POLINE").getMbo(i)) !=null); i++) {
						...
						mboDestino.setValue("MSALCODUNIDADEFORNECIMENTO", mbo.getString("ORDERUNIT"));
						System.out.println("############ ORDERUNIT = " + mbo.getString("ORDERUNIT"));
						mboDestino.setValue("MSCLOBCARACTERISTICAOBJETO", mbo.getString("MS_CARACTERISTICA"));
						System.out.println("############ MS_CARACTERISTICA = " + mbo.getString("MS_CARACTERISTICA"));
			...
			for (int i = 0; ((mbo1 = getMbo().getMboSet("MSTBITENSPREGAO").getMbo(i)) != null); i++) {
				...
				double valorunitariohomologado = mbo1.getDouble("MSNUNUMVALORUNITARIO");
				...
				if (mbo1.getString("MSALCODSITUACAO").equalsIgnoreCase("HOMOLOGADO")) {
					...
					if (mbo1.getMboSet("MSTBFORNECEDORESITEMPREGAO").count() > 0) {
						...
						for (int j = 0; ((mbo2 = mbo1.getMboSet("MSTBFORNECEDORESITEMPREGAO").getMbo(j)) != null); j++) {
							...
							valortotal += mbo2.getDouble("MSNUNUMVALORTOTAL");
							...
							if(valorunitariohomologado > 0d && mbo2.getDouble("MSNUNUMVALORUNITARIO") != valorunitariohomologado){
					...
					if(mbo1.getDouble("MSNUNUMVALORTOTAL") == valortotal){
						valorglobal += mbo1.getDouble("MSNUNUMVALORTOTAL");						
					} else {
						throw new MXApplicationException("pregao", "ValorTotalDoItemDivergente");
					}
            ...
            Logger.getLogger(MsPregSaa.class.getName()).log(Level.SEVERE, null, ex);
        }
	}
}
