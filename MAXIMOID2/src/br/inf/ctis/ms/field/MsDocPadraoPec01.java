	...
	public void portaria() throws MXException, RemoteException{
		...
		MboRemote inex = getMboValue().getMbo().getMboSet("PO").getMbo(0);
		 ...
		 StringBuilder val = new StringBuilder(); 
		    ...
		    val.append("<p style=\"text-align: justify;\">	Art 1.° - 	Designar, com base na confirmação da Tarefa 94 do PEC - Termo de Referência n.°"+inex.getInt("PONUM")+", o servidor, matricula SIAPE n.°, e como substituta eventual a servidora , mtrícula SIAPE n.°, como representantes do Minstério da Saúde para acompanhar e fiscalizar a execução do Contrato n.° [parametro], firmado com a empresa[parametro], que tem por objeto a aquisição do medicamento[parametro] [parametro] [parametro], resultante da Inexigibilidade de Licitação n.° [parametro], conforme Processo Eletrônico de Compras n.° "+inex.getInt("MS_SIPARNUM")+" - Termo de referência "+inex.getInt("PONUM")+"</p>");
