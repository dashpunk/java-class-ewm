 ...
 *
 */
public class ID2Map06 extends AppBean {
    ...
    private void validarSalvar() throws MXException, RemoteException {
        ...
        MboSet mboMaProcon = (MboSet) getMbo().getMboSet("MARL01RECE");
        if (getMbo().getBoolean("ID2SEMCON48") == false) {
            if (mboMaProcon.count() <= 0 && getMbo().getString("ID2STAMAP48").equalsIgnoreCase("CONCLUÍDO")) {
                throw new MXApplicationException("id2mapa06", "condProdutoNull");
            }

            if (!preencimentoObrigatorio((MboSet) getMbo().getMboSet("MARL01RECE")) && getMbo().getString("ID2STAMAP48").equalsIgnoreCase("CONCLUÍDO")) {
                throw new MXApplicationException("id2mapa06", "condProdutoNull");
            }
        }
        super.save();
