 ...
 * @author Patrick
 */
public class VistoriaTerminoObras extends psdi.webclient.system.beans.AppBean {
    ...
    public int ROUTEWF() throws MXException, RemoteException {
        ...
        mboSR = getMbo().getMboSet("MARL01WOR").getMbo(0).getMboSet("MARL05MEMOS");
...
//            throw new MXApplicationException("agendarvistoriaterreno", "RelacionamentoMARL05MEMOSSemRegistros");
        ...
        if (!preencimentoObrigatorio((MboSet) getMbo().getMboSet("MARL01WOR").getMbo(0).getMboSet("MARL05MEMOS"), "ID2LID", 1)) {
...
//            throw new MXApplicationException("agendarvistoriaterreno", "RelacionamentoMARL05MEMOSLider");
        ...
        MboRemote mbo;
        for (int i = 0; ((mbo = getMbo().getMboSet("MARL01WOR").getMbo(i)) != null); i++) {
            //Início Programado
        	System.out.println("*** antes MADTPROINI "+mbo.getDate("MADTPROINI"));
            if (mbo.isNull("MADTPROINI")) {
            	System.out.println("*** dentro IF MADTPROINI "+mbo.getDate("MADTPROINI"));
                setResultadoCampoObrigatorio("Início Programado");
                //throw new MXApplicationException("agendarvistoriaterreno", "MADTPROINIIsNull");
            }
            //Término Programado
            System.out.println("*** antes MADTPROFIM "+mbo.getDate("MADTPROFIM"));
            if (mbo.isNull("MADTPROFIM")) {
            	System.out.println("*** dentro IF MADTPROFIM "+mbo.getDate("MADTPROFIM"));
...
//            Logger.getLogger(VistoriaTerminoObras.class.getName()).log(Level.SEVERE, null, ex);
...
//    }
    ...
    public synchronized boolean fetchRecordData() throws MXException,
    		RemoteException {
    	getMbo().setValue("ID2NUMOS", getMbo().getMboSet("MARL01WOR").getMbo(0).getString("WONUM"));
