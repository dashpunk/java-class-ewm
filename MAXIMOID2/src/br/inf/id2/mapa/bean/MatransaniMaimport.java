 ...
 *
 */
public class MatransaniMaimport extends DataBean {
    ...
    private void tratamentoImportacao() throws MXException, RemoteException {
        ...
        String malocalori = app.getDataBean("MAINRECORD").getMbo().getString("MALOCALDEST");
        
        for (int linha = 0; linha < retornoExcel.size(); linha++) {

        	assetNum = (String) ((List) retornoExcel.get(linha)).get(0);

        	assetSet.setWhere("ASSETNUM = \'"+assetNum.trim()+"\' AND STATUS = 'EM USO' AND LOCATION IN (SELECT STORELOC FROM PO where ID2STORELOCDEST = \'"+app.getDataBean("MAINRECORD").getMbo().getString("MALOCALDEST")+"\') and MAESTRATIFICACAO IN (SELECT ITEMNUM FROM POLINE WHERE PONUM IN (SELECT PONUM FROM PO where ID2STORELOCDEST = \'"+app.getDataBean("MAINRECORD").getMbo().getString("MALOCALDEST")+"\'))");
            assetSet.reset();
            ...
            if (assetSet.count() > 0) {
            	...
            	mbo.setValue("MALOCALDEST", malocalori, MboConstants.NOVALIDATION_AND_NOACTION);

            	
            	mbo.setValue("ASSETNUM", assetNum, MboConstants.NOACCESSCHECK);
